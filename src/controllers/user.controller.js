...
const generateAccessAndRefreshTokens = async (userId) => {
  try {
    ...
    await user.save({ validateBeforeSave: false }); //so that they dont ask for password again
...
const registerUser = asyncHandler(async (req, res) => {
  ...
  const createdUser = await User.findById(user._id).select(
    "-password -refreshToken" //only items u dont need,
  );
...
const loginUser = asyncHandler(async (req, res) => {
  ...
  if (!isPasswordValid) {
    throw new ApiError(401, "Wrong Password or Password is invalid");
  }
...
const updateAccountDetails = asyncHandler(async (req, res) => {
  ...
  const user = User.findByIdAndUpdate(
    req.user?._id,
    {
      $set: {
        fullName,
        email, //email:email --> email es6 syntax
      },
    },
...
const getUserChannelProfile = asyncHandler(async (req, res) => {
  ...
  const channel = await User.aggregate([
    ...
    },
    {
      $addFields: {
        subscribersCount: {
          $size: { $ifNull: ["$subscribers", []] }, // safe for missing
        },
